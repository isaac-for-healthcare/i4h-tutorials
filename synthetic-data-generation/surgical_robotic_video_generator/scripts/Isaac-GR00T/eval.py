...
def calc_mse_for_single_trajectory(
    ...
    mse_xyz1 = np.mean((gt_action_across_time[:, :3] - pred_action_across_time[:, :3]) ** 2)
    mse_xyz2 = np.mean((gt_action_across_time[:, 10:13] - pred_action_across_time[:, 10:13]) ** 2)
    mse_jaw1 = np.mean((gt_action_across_time[:, 9:10] - pred_action_across_time[:, 9:10]) ** 2)
    mse_jaw2 = np.mean((gt_action_across_time[:, 19:20] - pred_action_across_time[:, 19:20]) ** 2)
    mse_rot1 = np.mean((gt_action_across_time[:, 3:9] - pred_action_across_time[:, 3:9]) ** 2)
    mse_rot2 = np.mean((gt_action_across_time[:, 13:19] - pred_action_across_time[:, 13:19]) ** 2)
    additional_info = {
    ...
    print("Additional MSE info:", additional_info)
...
def plot_trajectory(
    ...
    for i, ax in enumerate(axes):
        # The dimensions of state_joints and action are the same only when the robot
        # uses actions directly as joint commands. Therefore, do not plot them if not.
