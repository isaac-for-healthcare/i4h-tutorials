...
echo "  2. Clean up logs to free disk space (syslog will be permanently cleared — with confirmation)"
...
SYSLOG_SIZE=$(du -sh /var/log/syslog /var/log/syslog.1 2>/dev/null | awk '{sum += $1} END {print sum "K"}')
echo -e "${YELLOW}Syslog files currently use: ${SYSLOG_SIZE}${NC}"
echo -e "${RED}WARNING: Truncating syslog will permanently delete all syslog history.${NC}"
read -p "Truncate syslog files? (yes/no): " TRUNCATE_CONFIRM
if [ "$TRUNCATE_CONFIRM" = "yes" ]; then
    truncate -s 0 /var/log/syslog 2>/dev/null || true
    truncate -s 0 /var/log/syslog.1 2>/dev/null || true
    echo -e "${GREEN}✓ Syslog files truncated${NC}"
else
    echo -e "${YELLOW}Skipped syslog truncation${NC}"
fi
...
# Create symlink from /root/.cache/pip only if it does not exist (do not remove or move existing cache)
if [ "$LARGEST_PATH" != "/" ]; then
    mkdir -p /root/.cache
    if [ ! -e /root/.cache/pip ]; then
        ln -sf "$PIP_CACHE_DIR" /root/.cache/pip
        echo "Created symlink: /root/.cache/pip -> $PIP_CACHE_DIR"
    else
        echo "Leaving existing /root/.cache/pip unchanged"
    fi
fi
...
# =============================================================================
# Create .env for 02-guided_cosmos_2.5_setup.sh (paths for container setup)
# =============================================================================
echo -e "\n${GREEN}=== Cosmos Transfer 2.5 .env ===${NC}"
COSMOS_TUTORIAL_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
COSMOS_ENV="${COSMOS_TUTORIAL_DIR}/.env"
if [ "$LARGEST_PATH" = "/" ]; then
    COSMOS_BASE="/root"
else
    COSMOS_BASE="$LARGEST_PATH"
fi
if [ ! -f "$COSMOS_ENV" ]; then
    echo "Creating .env from template (paths based on largest drive: $COSMOS_BASE)"
    cat > "$COSMOS_ENV" << ENVEOF
# Cosmos Transfer 2.5 – paths (created by 01-system-setup.sh; edit if needed)
# 02-guided_cosmos_2.5_setup.sh sources this for repo and cache locations.

COSMOS_REPO_DIR=$COSMOS_BASE/cosmos-transfer2.5
COSMOS_HF_HOME=$COSMOS_BASE/.cache/huggingface
COSMOS_VENV_DIR=$COSMOS_BASE/.venv_guided_transfer2.5
COSMOS_TORCH_HOME=$COSMOS_BASE/.cache/torch
COSMOS_PIP_CACHE_DIR=$COSMOS_BASE/.cache/pip
COSMOS_ROBOTIC_US_DIR=$COSMOS_BASE/robotic_us_example
COSMOS_UV_CACHE_DIR=$COSMOS_BASE/.cache/uv
ENVEOF
    echo "Created: $COSMOS_ENV"
else
    echo "Existing .env left unchanged: $COSMOS_ENV"
fi
...
echo -e "${GREEN}========================================${NC}"
echo ""
echo -e "${CYAN}Next step: Run 02-guided_cosmos_2.5_setup.sh as your regular user (it will use .env)${NC}"
echo ""
